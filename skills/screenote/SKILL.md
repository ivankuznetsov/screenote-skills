...
- If the argument starts with `feedback` → tell the user: "Feedback has moved. Run `/feedback` (or `/screenote:feedback`) instead." Stop.
...
This section is the canonical capture-and-CLI-upload procedure. `/snapshot` reuses it for a multi-route manifest.
